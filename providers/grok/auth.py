...
BROWSER_TOKEN_ALIASES = {"grok-browser", "grok-cookies", "x-browser", "x-cookies"}

_GROK_BROWSER_AUTH = False  # TODO: read from env when implemented
...
class GrokAuth(BaseAuth):
    ...
    def ensure_browser_auth_request_allowed(self, request: Any = None) -> None:
        if not _GROK_BROWSER_AUTH:
            raise HTTPException(
                status_code=403,
                detail="Grok browser cookie auth is not yet implemented.",
            )

    async def get_access_token(self, profile: Optional[str] = None) -> str:
        raise HTTPException(status_code=501, detail="Grok browser auth not yet implemented.")
